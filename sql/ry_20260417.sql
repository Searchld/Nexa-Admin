...
-- 初始化-部门表数据
-- ----------------------------
insert into sys_dept values(100,  0,   '0',          '若依科技',   'RUOYI',      0, '若依', '15888888888', 'ry@qq.com', '0', '0', 'admin', sysdate(), '', null);
insert into sys_dept values(101,  100, '0,100',      '深圳总公司', 'SZ',         1, '若依', '15888888888', 'ry@qq.com', '0', '0', 'admin', sysdate(), '', null);
insert into sys_dept values(102,  100, '0,100',      '长沙分公司', 'CS',         2, '若依', '15888888888', 'ry@qq.com', '0', '0', 'admin', sysdate(), '', null);
insert into sys_dept values(103,  101, '0,100,101',  '研发部门',   'SZ_RD',      1, '若依', '15888888888', 'ry@qq.com', '0', '0', 'admin', sysdate(), '', null);
insert into sys_dept values(104,  101, '0,100,101',  '市场部门',   'SZ_MARKET',  2, '若依', '15888888888', 'ry@qq.com', '0', '0', 'admin', sysdate(), '', null);
insert into sys_dept values(105,  101, '0,100,101',  '测试部门',   'SZ_QA',      3, '若依', '15888888888', 'ry@qq.com', '0', '0', 'admin', sysdate(), '', null);
insert into sys_dept values(106,  101, '0,100,101',  '财务部门',   'SZ_FIN',     4, '若依', '15888888888', 'ry@qq.com', '0', '0', 'admin', sysdate(), '', null);
insert into sys_dept values(107,  101, '0,100,101',  '运维部门',   'SZ_OPS',     5, '若依', '15888888888', 'ry@qq.com', '0', '0', 'admin', sysdate(), '', null);
insert into sys_dept values(108,  102, '0,100,102',  '市场部门',   'CS_MARKET',  1, '若依', '15888888888', 'ry@qq.com', '0', '0', 'admin', sysdate(), '', null);
insert into sys_dept values(109,  102, '0,100,102',  '财务部门',   'CS_FIN',     2, '若依', '15888888888', 'ry@qq.com', '0', '0', 'admin', sysdate(), '', null);
...
create table sys_menu (
  ...
  is_frame          int(1)          default 1                  comment '链接打开模式（0内嵌 1外链）',
...
-- ----------------------------
-- 一级菜单
insert into sys_menu values('9000', '仪表盘', '0', '1', 'dashboard',        null, '', 'Dashboard', 1, 0, 'M', '0', '0', '', 'dashboard', 'admin', sysdate(), '', null, '仪表盘目录');
...
insert into sys_menu values('9003', '电子商务', '9000', '3', 'ecommerce', 'dashboard/ecommerce/index', '', 'Ecommerce', 1, 1, 'C', '0', '0', '', 'ecommerce', 'admin', sysdate(), '', null, '仪表盘电子商务菜单');
...
insert into sys_config values(10, '账号自助-验证码类型',          'sys.account.captchaType',          'image',         'Y', 'admin', sysdate(), '', null, '登录验证码类型（slider滑块 image图片）');
...
-- Art Design Pro route metadata
alter table sys_menu add column external_link varchar(500) default '' comment '外部链接';
alter table sys_menu add column show_badge char(1) default '0' comment '显示圆点徽章（0否 1是）';
alter table sys_menu add column show_text_badge varchar(50) default '' comment '文本徽章';
alter table sys_menu add column is_hide_tab char(1) default '0' comment '隐藏标签页（0否 1是）';
alter table sys_menu add column fixed_tab char(1) default '0' comment '固定标签页（0否 1是）';
alter table sys_menu add column active_path varchar(200) default '' comment '激活菜单路径';
alter table sys_menu add column is_full_page char(1) default '0' comment '全屏页面（0否 1是）';
alter table sys_menu add column new_tab char(1) default '0' comment '新标签打开（0否 1是）';

update sys_menu set external_link = path where path regexp '^https?://';
update sys_menu
set icon = case icon
  when 'dashboard' then 'ri:pie-chart-line' when 'system' then 'ri:settings-3-line'
  when 'monitor' then 'ri:computer-line' when 'tool' then 'ri:tools-line'
  when 'guide' then 'ri:guide-line' when 'console' then 'ri:home-smile-2-line'
  when 'analysis' then 'ri:bar-chart-box-line' when 'ecommerce' then 'ri:shopping-bag-3-line'
  when 'user' then 'ri:user-line' when 'peoples' then 'ri:team-line'
  when 'tree-table' then 'ri:menu-line' when 'tree' then 'ri:node-tree'
  when 'post' then 'ri:briefcase-line' when 'dict' then 'ri:book-2-line'
  when 'edit' then 'ri:edit-line' when 'message' then 'ri:notification-3-line'
  when 'log' then 'ri:file-list-3-line' when 'online' then 'ri:global-line'
  when 'job' then 'ri:timer-line' when 'server' then 'ri:server-line'
  when 'redis' then 'ri:database-2-line' when 'redis-list' then 'ri:list-check-3'
  when 'build' then 'ri:tools-line' when 'code' then 'ri:code-s-slash-line'
  when 'swagger' then 'ri:file-code-line' when 'form' then 'ri:file-list-3-line'
